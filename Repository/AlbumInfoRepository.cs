...
public class AlbumInfoRepository(IDbConnection connection) : IAlbumInfoRepository
{
    ...
    public async Task<int> SaveAsync(List<AlbumInfo> albums)
    {
        ...
        foreach (var album in albums)
        {
            var builder = new SqlBuilder();
            var artistInsert = 
                builder.AddTemplate("INSERT OR IGNORE INTO AlbumInfo (Id, Title,FirstReleaseDate, Description, Image, ArtistsInfoId) " +
                                    "VALUES (@Id,ArtistsInfoId @Title, @FirstReleaseDate, @Description, @Image, @)"); 
